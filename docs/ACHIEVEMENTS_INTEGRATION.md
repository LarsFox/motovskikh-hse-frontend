...
Поля `title` и `description` используют существующий контракт сайта `I18n`. Например,
ответ на запрос с `language=ru` содержит `{"title":{"ru":"Первая отметка"}}`, а
компонент выбирает строку по текущему языку страницы. Временная поддержка строковых
значений сохранена для совместимости с тестовым backend во время перехода.
